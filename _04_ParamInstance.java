...
class Updater2 {
    void update2(Counter counter) { // 따라서 메소드의 입력으로 Counter 클래스의 객체를 받고
        counter.count++; // 객체 counter의 객체변수 count에 1을 더하면
                         // 즉, myCounter의 객체변수 count에 1을 더하면 (-> 29번째 줄)
    }
}
...
public class _04_ParamInstance {
    public static void main(String[] args) {
        ...
        System.out.println("before update:"+myCounter.count); // 0이 출력됨
        ...
        myUpdater2.update2(myCounter); // update2 메소드에 객체(myCounter)를 전달하면 해당 메소드는 전달받은 객체를 그대로 수행하여
                                       // 메소드가 객체(myCounter)의 속성값을 변경하면 메소드 수행 후에도 객체의 변경된 속성값이 유지된다.
